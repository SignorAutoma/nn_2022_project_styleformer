...
def modulated_channel_attention(x, q_weight, k_weight, v_weight, w_weight, u_weight, proj_weight, styles, num_heads):
    ...
    attention_scale = torch.tensor(depth ** -0.5).to(x.dtype) # Try to use torch.as_tensor to improve performances CHECK https://pytorch.org/docs/stable/generated/torch.as_tensor.html#torch.as_tensor
    ...
    # Reduce complexity through linformer
...
# The purpose of the mapping network is to produce the latent input vector within 
# the intermediate vector whose distinctive element control various visual features.
# Instead of immediately implementing latent vector to the input layer, mapping is done.

# N.B. MAPPING NETWORK IS THE SAME AS STYLEGAN IMPL!!!!
...
class Encoderlayer(nn.Module):
    def __init__(self, h_dim, w_dim, out_dim, seq_length, depth, minimum_head, use_noise=True, conv_clamp=None,
    ...
    def forward(self, x, w, noise_mode='random', gain=1):
        ...
        x = modulated_channel_attention(x=x, q_weight=self.q_weight, k_weight=self.k_weight, v_weight=self.v_weight,
                                        w_weight=self.w_weight, u_weight=self.u_weight, proj_weight=self.proj_weight,
    ...
    def compute_qkv_weights(self, h_dim, memory_format):
        self.q_weight = torch.nn.Parameter(
            torch.FloatTensor(h_dim, h_dim).uniform_(-1. / math.sqrt(h_dim), 1. / math.sqrt(h_dim)).to(memory_format=memory_format))
...
# Generates an RGB image from a feature map
...
class ToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, conv_clamp=None, channels_last=False):
        ...
        self.weight = torch.nn.Parameter(
            torch.FloatTensor(out_channels, in_channels).uniform_(-1. / math.sqrt(in_channels), 1. / math.sqrt(in_channels)).to(memory_format=memory_format))
...
class EncoderBlock(nn.Module):
    def __init__(self, h_dim, w_dim, out_dim, depth, minimum_head, img_resolution, resolution, img_channels, is_first,
                 is_last, architecture='skip', linformer=True, conv_clamp=None, use_fp16=False,
                 fp16_channels_last=False, resample_filter=[1, 3, 3, 1], scale_ratio=2, **layer_kwargs):
        super().__init__()
        self.h_dim = h_dim
        self.w_dim = w_dim
        self.out_dim = out_dim
        ...
        self.applyLineformer(memory_format) # We need to boost the performance so we always apply lineformer

        self.handleFirstLayer()

        if not self.is_last or out_dim is None:
            self.out_dim = h_dim # Every output has the resolution of the related hidden dimension.

        self.enc = Encoderlayer(h_dim=self.h_dim, w_dim=self.w_dim, out_dim=self.out_dim, seq_length=self.seq_length,
                                depth=self.depth, minimum_head=self.minimum_head, conv_clamp=self.conv_clamp,
                                proj_weight=self.proj_weight)
        self.num_attention += 1

        if self.is_last and self.architecture == 'skip':
            self.torgb = ToRGBLayer(self.out_dim, self.img_channels, w_dim=w_dim, conv_clamp=conv_clamp,
    ...
    def forward(self, x, img, ws, force_fp32=True, fused_modconv=None):
        ...
        if self.architecture == 'resnet':
            # Note that y is related to toRGB result while x is the result of EncoderLayer (that includes also the input of the layer itself)
        ...
        # ToRGB
        x, img = self.assembleRGB(x, img, ws, fused_modconv, w_iter)

        assert x.dtype == dtype
        assert img is None or img.dtype == torch.float32
        return x, img

    def applyLineformer(self, memory_format):
        if self.resolution >= 32: 
            self.proj_weight = torch.nn.Parameter(
                torch.FloatTensor(256, self.seq_length).uniform_(-1. / math.sqrt(self.seq_length), 1. / math.sqrt(self.seq_length)).to(memory_format=memory_format))

    def handleFirstLayer(self):
        if self.is_first and self.resolution == 8:
    ...
    def assembleRGB(self, x, img, ws, fused_modconv, w_iter):
        if self.is_last:
            # Perform bilinear upsampling which means reconstruct missing pixels
            # after a resize through linear interpolation. 
            if img is not None:
                misc.assert_shape(img, [None, self.img_channels, self.resolution // 2, self.resolution // 2])
                img = upfirdn2d.upsample2d(img, self.resample_filter)

            if self.architecture == 'skip':
                img = self.performSkipConnection(x, img, ws, fused_modconv, w_iter)
            # upsample
            if self.resolution != self.img_resolution:
                x = upfirdn2d.upsample2d(
                    x.permute(0, 2, 1).reshape(ws.shape[0], self.out_dim, self.resolution, self.resolution),
                    self.resample_filter)
                x = x.reshape(ws.shape[0], self.out_dim, self.seq_length * self.scale_ratio ** 2).permute(0, 2, 1)
        return x,img

    def performSkipConnection(self, x, img, ws, fused_modconv, w_iter):
        y = self.torgb(x.permute(0, 2, 1).reshape(ws.shape[0], self.out_dim, self.resolution, self.resolution),
                               next(w_iter), fused_modconv=fused_modconv)
...
class SynthesisNetwork(nn.Module):
    def __init__(self,
        ...
        depth,             # Depth for transformer encoder layer
        minimum_head,      # Minimum_head for transformer encoder layer
        num_layers,        # Number of Transformer encoder layer.
        G_dict,            # Hidden dimension for transformer encoder layer
        conv_clamp,        # Clamp the output of convolution layers to +-X, None = disable clamping.
        ...
        linformer=True     # Lightweightening the result
    ):
        self.checkAllowedResoultion(img_resolution)
        super().__init__()
        self.w_dim = w_dim
        self.img_resolution = img_resolution
        ...
        # N.B. the w used by the toRGB layer is the same of the EncoderLayer in the next block
        self.num_ws = 0 
        # This process is repeated until the feature map resolution 
        # reaches the target img resolution.
        for i, res in enumerate(self.block_resolutions):
            ...
            for j in range(num_layers_block):
                is_first = (j == 0)
                is_last = (j == num_layers_block - 1)
                # Each block has 2 layers, an EncoderLayer and a RGB layer
               
                block = EncoderBlock(
                    h_dim=h_dim, w_dim=w_dim, out_dim=out_dim, depth=depth, minimum_head=minimum_head,
                    img_resolution=img_resolution, resolution=res, img_channels=img_channels,
    ...
    def checkAllowedResoultion(self, img_resolution):
...
class Generator(nn.Module):
    def __init__(self, z_dim, c_dim, w_dim, img_resolution, img_channels, mapping_kwargs={}, synthesis_kwargs={}):
        super().__init__()
        self.z_dim = z_dim # Input Latent dim
        self.c_dim = c_dim # Conditional label dim
        self.w_dim = w_dim # Intermediate Latent dim
        self.img_resolution = img_resolution # Out res
        self.img_channels = img_channels # Number of color chan
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, epoch=None, **synthesis_kwargs):
        # Essentially here we are passing the MappingNetwork to the Syntesys Network.
        # MappingNetwork is used to adjust weights based on gradients
